...
<script>
import { getProductCategory, createProductCategory, deleteProductCategory, updateProductCategory } from '@/api/product'
...
export default {
  ...
  methods: {
    getList() {
      ...
      getProductCategory(this.listQuery).then(response => {
        this.list = response
    ...
    updateData() {
      this.$refs['dataForm'].validate((valid) => {
        if (valid) {
          ...
          updateProductCategory(tempData, tempData._id).then(() => {
    ...
    handleDownload() {
      ...
      import('@/vendor/Export2Excel').then(excel => {
        const tHeader = ['name', 'value', 'status']
        const filterVal = ['name', 'value', 'status']
